...
folder_path = "C:/Users/UMSS/Documents/Dispa-LINK/Inputs/SDDP"
...
hydro = pd.read_csv(str(folder_path)+'/chidrobo.csv', usecols=['...Nombre...','.VMax..','.VInic.','....Pot','.FPMed.','C.Vert.','.PV.','.QMax..'], header=0)
...
buslist = pd.read_csv(str(folder_path)+'/BusList-ZONES1.csv', encoding='cp1252')

#Inputs outages
#thermal
dft1 = pd.read_csv(str(folder_path)+'/pmtrsebo.csv', encoding='cp1252')
dft2 = pd.read_csv(str(folder_path)+'/ctermibo.csv', usecols=['...Nombre...','..Ih...','.PotIns','Comb'])

#hydro
dfh1 = pd.read_csv(str(folder_path)+'/pmhisebo.csv', encoding='cp1252')
# #vres
dfv1 = pd.read_fwf(str(folder_path)+'/cgndbo.dat', header=1, colspecs="infer", engine='python',usecols=['!Num Name........','ProbFal','.PotIns'])

dbus = pd.read_csv(str(folder_path)+'/dbus.csv',encoding='cp1252', header=1, usecols= ['Name','Gen. name'])
...
Outages = dl.get_Outages(dft1,dft2, dfh1,dfv1, start_date, end_date)
