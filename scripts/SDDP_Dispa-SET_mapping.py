...
#Results
alertlev = dl.get_alert_level(hydro,SDDP_alert, start_date=start_date, end_date=end_date)
finaldemand = dl.get_demand(SDDP_demand, dfc, buslist)
ntc = dl.get_NTC(dfc)
Outages = dl.get_Outages(dft1,dft2, dfh1,dfv1)
PowerPlantData = dl.get_Power_PlantData(hydro,dft2,dfv1,dbus,buslist)
RenewablesAF = dl.get_renewables_AF(vresaf)
Spilcost = dl.get_Spillage_Cost(hydro)
reserlevh = dl.get_Storage_lev(SDDP_lev,hydro)
#Scaled Inflows
